		...
		for (int i = 0; i < numLines; i++){
			...
			if(cacheMeta[index][0] == tag && cacheMeta[index][1] == 1){
				...
				//write hit
					...
					cacheMeta[index][2] = 1;	//set dirty bit
					cacheBlock[index][offset<<2] = data[i]; //write to cache
				}
			}
			//cache miss
			else{
				verboseStream << "0 2";
				dataMiss++;
				cacheMeta[index][0] = tag;	//set tag
				cacheMeta[index][1] = 1;	//set valid bit
				cacheBlock[index][offset<<2] = data[i]; //write to cache
				...
				if(cacheMeta[index][2] == 0){
					...
					if(accessType[i].compare("R") == 0){
						...
						readTime+= 81; 
						bytesRead+= blockSize;
						cacheMeta[index][2] = 0; //set dirty bit off
					}
					//clean write miss
					else{
						writes++;
						...
						writeTime+= 81; 	
						bytesRead+= blockSize;
						cacheMeta[index][2] = 1;	//set dirty bit on
					}
				}
				//dirty miss
				else{
